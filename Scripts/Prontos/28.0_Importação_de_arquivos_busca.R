######################################################################################################################################################
######################################################################################################################################################

#IMPORTAÇÃO DE ARQUIVOS - BUSCA

######################################################################################################################################################
######################################################################################################################################################

#-----------------------------------------------------------------------------------------------------------------------------------------------------
#/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#BUSCANDO UM ARQUIVO COM file.choose() 

#Buscar um arquivo pelo computador

  file.choose()

#-----------------------------------------------------------------------------------------------------------------------------------------------------  
#A função file.choose() devolve o caminho do arquivo buscado

#Pode ser armazenado numa variável

  arquivo <- file.choose()  

#-----------------------------------------------------------------------------------------------------------------------------------------------------
#/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#BUSCANDO UM ARQUIVO COM file.path() 

#Devolve um caminho de arquivo
  
#Cada vírgula delimita um diretório (nunca esquecer a extensão do arquivo)

  caminho <- file.path("E:","GITHUB","RProject","Arquivos", "Arquivos_usados_scripts", "vendas_supermercado.csv")

#----------------------------------------------------------------------------------------------------------------------------------------------------- 
#Por padrão, o separador é a vírgula, mas podemos definir outro

  caminho <- file.path("E:","GITHUB","RProject","Arquivos", "Arquivos_usados_scripts", "vendas_supermercado.csv", fsep = "*")
  
  caminho <- file.path("E:","GITHUB","RProject","Arquivos", "Arquivos_usados_scripts", "vendas_supermercado.csv", fsep = "/") 

#-----------------------------------------------------------------------------------------------------------------------------------------------------